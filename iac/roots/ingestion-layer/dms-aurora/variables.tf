...
variable "AURORA_FINANCIAL_TOPIC_NAME_PARAMETER" {
  description = "Name of the Aurora financial topic parameter"
  type        = string
}

variable "AURORA_BROKERAGE_TOPIC_NAME_PARAMETER" {
  description = "Name of the Aurora brokerage topic parameter"
  type        = string
}

variable "AURORA_MSK_MIGRATION_TYPE" {
  description = "Aurora to MSK migration type (full-load, cdc, full-load-and-cdc)"
  type        = string
  default     = "cdc"

  validation {
    condition     = contains(["full-load", "cdc", "full-load-and-cdc"], var.AURORA_MSK_MIGRATION_TYPE)
...
variable "START_AURORA_REPLICATION_TASK" {
  description = "Start Aurora replication task after creation"
  type        = bool
  default     = false
}
