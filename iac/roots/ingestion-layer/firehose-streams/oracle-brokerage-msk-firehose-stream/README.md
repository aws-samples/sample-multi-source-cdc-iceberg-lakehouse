# Oracle Brokerage MSK Firehose Stream

## Purpose
Creates a Kinesis Data Firehose delivery stream that ingests Oracle Database brokerage transaction data from MSK topics into Apache Iceberg tables, with Lambda transformation for DMS CDC envelope flattening.

## What It Creates
- **Firehose Delivery Stream**: `oracle-brokerage-msk-stream` for real-time data ingestion
- **Lambda Transformer**: Flattens DMS CDC envelope format to flat brokerage records
- **Glue Table Integration**: `oracle_brokerage_transactions_table` in Oracle database
...
- **Real-time Ingestion**: Streams brokerage transactions from Oracle to data lake
...
FIREHOSE_STREAM_NAME = "oracle-brokerage-msk-stream"
ORACLE_BROKERAGE_TABLE_NAME = "oracle_brokerage_transactions_table"
...
## Data Flow
1. **Oracle Database** → DMS captures brokerage transaction changes
...
- MSK cluster with `msk-ingest-oracle-brokerage-transactions` topic
- Glue database and table definitions
- Lambda transformer function

## Monitoring
- **CloudWatch Logs**: `/aws/firehose/${APP_NAME}-${ENV_NAME}-oracle-brokerage-msk-stream`
