# Aurora Financial MSK Firehose Stream

## Purpose
Creates a Kinesis Data Firehose delivery stream that ingests Aurora PostgreSQL financial transaction data from MSK topics into Apache Iceberg tables, with Lambda transformation for DMS CDC envelope flattening.

## What It Creates
- **Firehose Delivery Stream**: `aurora-financial-msk-stream` for real-time data ingestion
- **Lambda Transformer**: Flattens DMS CDC envelope format to flat financial records
- **Glue Table Integration**: `aurora_financial_transactions_table` in Aurora database
...
- **Real-time Ingestion**: Streams financial transactions from Aurora to data lake
...
FIREHOSE_STREAM_NAME = "aurora-financial-msk-stream"
AURORA_FINANCIAL_TABLE_NAME = "aurora_financial_transactions_table"
...
## Data Flow
1. **Aurora PostgreSQL** → DMS captures financial transaction changes
...
- MSK cluster with `msk-ingest-aurora-financial-transactions` topic
- Glue database and table definitions
- Lambda transformer function

## Monitoring
- **CloudWatch Logs**: `/aws/firehose/${APP_NAME}-${ENV_NAME}-aurora-financial-msk-stream`
