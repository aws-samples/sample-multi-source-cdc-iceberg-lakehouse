...
data "aws_ssm_parameter" "msk_ingest_aurora_financial_transactions_topic_name" {
  name = "/${var.APP}/${var.ENV}/${var.AURORA_FINANCIAL_TOPIC_NAME_PARAMETER}"
}

data "aws_ssm_parameter" "aurora_database_name" {
  name = "/${var.APP}/${var.ENV}/${var.AURORA_DATABASE_NAME_PARAMETER}"
}
...
locals {
  ...
  AURORA_DATABASE_NAME  = data.aws_ssm_parameter.aurora_database_name.value
  TOPIC_NAME            = data.aws_ssm_parameter.msk_ingest_aurora_financial_transactions_topic_name.value
