# Oracle Financial MSK Firehose Stream

## Purpose
Creates a Kinesis Data Firehose delivery stream that ingests Oracle Database financial transaction data from MSK topics into Apache Iceberg tables, with Lambda transformation for DMS CDC envelope flattening.

## What It Creates
- **Firehose Delivery Stream**: `oracle-financial-msk-stream` for real-time data ingestion
- **Lambda Transformer**: Flattens DMS CDC envelope format to flat financial records
- **Glue Table Integration**: `oracle_financial_transactions_table` in Oracle database
...
- **Real-time Ingestion**: Streams financial transactions from Oracle to data lake
...
FIREHOSE_STREAM_NAME = "oracle-financial-msk-stream"
ORACLE_FINANCIAL_TABLE_NAME = "oracle_financial_transactions_table"
...
## Data Flow
1. **Oracle Database** → DMS captures financial transaction changes
2. **DMS** → Publishes CDC envelopes to MSK topic
3. **MSK Topic** → Firehose stream consumes messages
4. **Lambda Transformer** → Flattens DMS envelope to financial record
5. **Iceberg Table** → Stores transformed data in S3 with Glue catalog

## Dependencies
- Foundation layer (KMS keys, S3 buckets, IAM roles)
- Oracle Database with DMS replication
- MSK cluster with `msk-ingest-oracle-financial-transactions` topic
- Glue database and table definitions
- Lambda transformer function

## Monitoring
- **CloudWatch Logs**: `/aws/firehose/${APP_NAME}-${ENV_NAME}-oracle-financial-msk-stream`
